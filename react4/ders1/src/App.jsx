...
function App() {
  ...
  const sepetsil = (urun) => {
    //let yenisepet = sepet.filter((u) => u.urun.id !== urun.id);
    //setsepet(yenisepet);
    let yenisepet = [...sepet];
                  ...
                  <DropdownMenu end>
                    {sepet.map((urun) => (
                      <DropdownItem key={urun.id}>
                        ...
                        {urun.productName}
                        <span onClick={() => sepetguncelle(urun.urun, "azalt")}>
                          -
                        </span>
                      </DropdownItem>
                    ))}
          ...
          <Col xs="3">
            <ListGroup>
              {kategoriler.map((kategori) => (
                <ListGroupItem
                  active={
                    kategori.categoryName == secilikategori ? true : false
                  }
                  key={kategori.id}
